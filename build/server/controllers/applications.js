...
var AppManager, Application, CheckUpdate, Manifest, NotificationsHelper, UpdateNotifier, fs, mark_broken, randomString, request, saveIcon, send_error, send_error_socket, slugify, startedApplications;
...
CheckUpdate = function(app) {
  var log, manifest, setFlag;
  log = function(t) {
  ...
  setFlag = (function(_this) {
    return function() {
      ...
      app.save(function(ers) {
        if (ers) {
          console.error("Error when setting the needsUpdate flag for " + app.name + ": " + ers);
          return;
        }
        return UpdateNotifier.createTemporary({
          text: "A new version of " + app.name + " has been released! Click on 'Manage your apps' to update it.",
          resource: {
            app: 'home'
          }
        });
  ...
  return manifest.download(app, function(erm) {
    var repoVersion;
    if (erm) {
      console.error("Error when downloading manifest for " + app.name + ": " + erm);
      return;
    }
...
setInterval(function() {
  ...
  Application.all(function(err, apps) {
    var app, _i, _len;
    if (err) {
